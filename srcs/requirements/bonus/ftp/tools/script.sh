...
echo "[ftp] Bootstrapping vsftpd"

mkdir -p /var/run/vsftpd/empty
mkdir -p "$WP_DIR"

echo "[ftp] Using data dir: $WP_DIR"

WWW_UID=$(id -u www-data)
WWW_GID=$(id -g www-data)

if ! id "$FTP_USER" >/dev/null 2>&1; then
	echo "[ftp] Adding user $FTP_USER (uid=$WWW_UID gid=$WWW_GID)"
	useradd -o -u "$WWW_UID" -g "$WWW_GID" -d /var/www/html/wordpress \
		-s /bin/bash "$FTP_USER"
fi

echo "${FTP_USER}:${FTP_PASS}" | chpasswd
echo "[ftp] Password updated for $FTP_USER"

echo "[ftp] Writing userlist"
...
if [ -f "/vsftpd.conf.template" ]; then
	...
	echo "[ftp] Config ready (pasv ${FTP_PASV_ADDRESS}:${FTP_PASV_MIN}-${FTP_PASV_MAX})"
fi

echo "[ftp] Starting FTP server"
